...
def build_model(w2v_model, rnn_size, vocab_size, n_vectors, seq_length, batch_size,
    ...
    model.add(LSTM(rnn_size, return_sequences=True, stateful=is_stateful))
    model.add(LSTM(rnn_size, return_sequences=True, stateful=is_stateful))
...
def train(model, w2v_model, epochs, batch_size, seq_length, x_train, y_train, x_val, y_val,
    ...
    model.fit(
        ...
        callbacks=[tensorboard, lr_reducer, model_saver, sequence_generator]
    )
...
def run():
    ...
    train(model, w2v_model, EPOCHS, BATCH_SIZE, SEQ_LENGTH, x_train, y_train, x_val, y_val, model_dir, MODEL_NAME)
